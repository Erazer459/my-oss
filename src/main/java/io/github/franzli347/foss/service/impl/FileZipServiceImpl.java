...
public class FileZipServiceImpl implements FileZipService {//TODO 大文件压缩后修改文件md5
    ...
    public void videoCompress(int vid, VideoCompressArgs args, String userId) {
        if (Optional.ofNullable(stringRedisTemplate.opsForSet().isMember(RedisConstant.COMPRESS_TASK + "_" + vid, args.toString()))
            ...
            throw new AsyncException( Result.builder().code(ResultCode.CODE_ERROR).msg("压缩任务已存在").build(),userId);
        }
        Files files = filesService.getById(vid);
        //校验视频压缩参数:码率,分辨率,帧率
        List<String> error = FileUtil.compressArgsLegal(files.getPath(), args);
        if (!error.isEmpty()){
            throw new AsyncException( Result.builder().code(ResultCode.CODE_ERROR).msg("视频压缩参数错误").data(error).build(),userId);}
        ...
        try {
            FileZipUtil.videoCompress(files.getPath(), args, info, evt -> {//持续监听压缩进度
                ...
                if (evt.getPropertyName().equals(DONE) && (boolean) evt.getNewValue()){
                    webSocketHandler.sendResultMsg(userId,Result.builder().msg("视频压缩完毕").code(ResultCode.CODE_SUCCESS).data(evt.getSource()).build());
                }
            });
        } catch (Exception e) {
            stringRedisTemplate.opsForSet().remove(RedisConstant.COMPRESS_TASK + "_" + info.getId(), args.toString());
            throw new AsyncException(Result.builder().code(ResultCode.CODE_ERROR).msg("视频压缩失败").data(vid).build(), userId);
        }
    ...
    public void imageCompress(int imageId, String userId) {
        String imagePath= String.valueOf(filesService.getById(imageId));
            if (Optional.ofNullable(stringRedisTemplate.opsForSet().isMember(RedisConstant.COMPRESS_TASK + "_" + imageId, imageId))
                    .orElse(false)) {
                throw new AsyncException(Result.builder().code(ResultCode.CODE_ERROR).msg("压缩任务已存在").data(imageId).build(),userId);
            }
            if (new File(imagePath).length() / (1024 * 1024) > 600) {//限定图片大小
                throw new AsyncException(Result.builder().code(ResultCode.CODE_ERROR).msg("图片大小不得超过600MB").data(imageId).build(),userId);
            }
        try {
            ...
            throw new AsyncException(Result.builder().code(ResultCode.CODE_ERROR).msg("图片压缩失败").data(imageId).build(),String.valueOf(userId));
        }
        webSocketHandler.sendResultMsg(userId, Result.builder().code(ResultCode.CODE_SUCCESS).msg("图片压缩成功").data(imageId).build());
        stringRedisTemplate.opsForSet().remove(RedisConstant.COMPRESS_TASK + "_" + imageId, imageId);
    }
}
